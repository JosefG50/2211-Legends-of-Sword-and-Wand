...
Endpoints:
  POST /battle/start          - Start a turn-based battle (PvP or PvE)
  POST /battle/action         - Submit a player action for the current turn
  GET  /battle/state/<id>     - Get current state of a turn-based battle
  POST /battle/run            - Auto-battle in one call (used by PvE service)
  POST /pvp/invite            - Send a PvP invitation
  POST /pvp/invite/<id>/respond - Accept or decline invitation
  GET  /pvp/invite/<id>       - Get invitation details
  GET  /pvp/invitations       - List invitations for a player
  GET  /pvp/league            - League standings
  GET  /pvp/league/<username> - One player's stats
...
app = Flask(__name__)
CORS(app)  # Allow requests from the frontend running in the browser
...
# -------------------------------------------------------
# Ability aliases — must be defined before apply_cast uses them
# -------------------------------------------------------
...
ABILITY_ALIASES = {
    ...
    "berserker_attack": "berserker_attack",
    "replenish":        "replenish",
    "fire_shield":      "protect",
    "sneak_attack":     "berserker_attack",
    "mana_burn":        "replenish",
}
...
def get_available_actions(hero):
    ...
    cn = hero.get("class_name", "Warrior")
    ability = DEFAULT_ABILITY.get(cn) or DEFAULT_ABILITY.get(cn.title(), "berserker_attack")
    # Also check abilities list sent from Party service
    hero_abilities = hero.get("abilities", [])
    if hero_abilities:
        for ab in hero_abilities:
            if ab.get("type") in ("spell", "special"):
                ab_key = ab["name"].lower().replace(" ", "_")
                cost = ab.get("mana_cost", ABILITY_MANA_COST.get(ab_key, 999))
                if hero["mana"] >= cost:
                    actions.append(f"cast:{ab_key}")
    else:
    ...
    if not battle_over:
        ...
        if active_hero:
            ...
            state["current_turn"] = {
                ...
                "valid_targets": [
                    {"name": e["name"], "index": e.get("index", 0),
...
def apply_cast(hero, ability, allies, enemies, target_index, log):
    # Normalise ability name (handle spaces, caps, aliases)
    ability = ABILITY_ALIASES.get(ability.lower().replace(' ', '_'), ability.lower().replace(' ', '_'))
    import random
    cost = ABILITY_MANA_COST.get(ability, 999)
    if hero["mana"] < cost:
    ...
    if ability == "protect":
        ...
        log.append(f"    {target['name']} healed for {amount} HP.")
        ...
        for i, t in enumerate([first] + rest):
            ...
            if dmg == 0:
                break
            t["hp"] = max(0, t["hp"] - dmg)
            log.append(f"    Chain Lightning hits {t['name']} for {dmg}.")
            if t["hp"] <= 0:
                log.append(f"    {t['name']} is defeated!")
        ...
        log.append(f"    Berserker Attack hits {main['name']} for {dmg}.")
        if main["hp"] <= 0:
            log.append(f"    {main['name']} is defeated!")
        others = [e for e in enemies if e is not main and e["hp"] > 0]
        for t in random.sample(others, min(2, len(others))):
        ...
        for ally in allies:
            ...
            log.append(f"    {ally['name']} restores mana.")
...
def start_turn_battle():
    ...
    Input:  { team_a, team_b, name_a, name_b, mode ("pvp"|"pve") }
    Output: Full game state with battle_id and current_turn.
    """
    ...
    state["battle_id"] = bid
    conn.execute("UPDATE active_battles SET state=? WHERE id=?", (json.dumps(state), bid))
    conn.commit()
    conn.close()
...
def submit_action():
    ...
    Input:  { battle_id, action, target_index }
    ...
    if active_hero.get("stunned"):
        ...
        apply_cast(active_hero, action.split(":", 1)[1], allies, enemies, target_index, log)
    else:
        conn.close()
    ...
    # In PvE mode: auto-play enemy turns until it's the player's turn again
    if mode == "pve":
        import random
        while True:
    ...
    state = build_state(battle_id, team_a, team_b,
                        state["name_a"], state["name_b"], log, turn_index, mode)
...
# -------------------------------------------------------
# AUTO-BATTLE — used by PvE service
# -------------------------------------------------------

@app.route("/battle/run", methods=["POST"])
def auto_battle():
    """
    Run a full battle automatically. Used by the PvE service.
    Input:  { team_a, team_b, name_a, name_b }
    Output: { winner, battle_log, team_a, team_b }
    """
    data   = request.get_json()
    team_a = data.get("team_a")
    team_b = data.get("team_b")
...
def send_invite():
    ...
    if existing:
        conn.close()
        return jsonify({"error": "A pending invitation already exists",
                        "invitation_id": existing["id"]}), 409

    cursor = conn.execute("INSERT INTO invitations (inviter, invitee) VALUES (?, ?)",
                          (inviter, invitee))
    conn.commit()
...
# -------------------------------------------------------
# ABILITIES LOOKUP — returns abilities for a class_name
# -------------------------------------------------------

# All abilities by class (title-cased class name)
CLASS_ABILITIES = {
    "Order":   [{"name": "Protect",          "key": "protect",          "mana_cost": 25, "type": "spell"},
                {"name": "Heal",             "key": "heal",             "mana_cost": 35, "type": "spell"}],
    "Chaos":   [{"name": "Fireball",         "key": "fireball",         "mana_cost": 30, "type": "spell"},
                {"name": "Chain Lightning",  "key": "chain_lightning",  "mana_cost": 40, "type": "spell"}],
    "Warrior": [{"name": "Berserker Attack", "key": "berserker_attack", "mana_cost": 60, "type": "special"}],
    "Mage":    [{"name": "Replenish",        "key": "replenish",        "mana_cost": 80, "type": "spell"}],
}


@app.route("/abilities/<class_name>", methods=["GET"])
def get_class_abilities(class_name):
    """Return abilities for a given class name (case-insensitive)."""
    cn = class_name.title()
    abilities = CLASS_ABILITIES.get(cn, [])
    return jsonify({"class_name": cn, "abilities": abilities}), 200


@app.route("/health", methods=["GET"])
def health():
    return jsonify({"status": "ok", "service": "battle-service"}), 200
